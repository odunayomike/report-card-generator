      ...
      if (editingExam) {
        ...
        const updateResponse = await fetch(`${API_BASE_URL}/cbt/exams`, {
          ...
          body: JSON.stringify({
            ...
            show_results_immediately: examForm.show_results ? 1 : 0,
    ...
    try {
      ...
      if (data.success) {
        ...
        setExamForm({
          ...
          show_results: Boolean(exam.show_results_immediately),
    ...
    <div>
      ...
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">Create and manage computer-based tests</p>
        </div>
        <button
          onClick={() => {
            setShowWizard(true);
          ...
          <div className="bg-white rounded-lg max-w-md w-full">
            <div className="p-6">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Publish Exam</h3>
                ...
                {['Exam Details', 'Select Questions', 'Assign Students', 'Review'].map((step, idx) => (
                  <div key={idx} className="flex items-center flex-1">
                    <div className={`flex items-center ${idx < 3 ? 'flex-1' : ''}`}>
                      ...
                      <span className={`ml-2 text-sm ${currentStep === idx + 1 ? 'font-semibold' : ''}`}>
                        {step}
                      </span>
                    </div>
                    {idx < 3 && (
                      <div className={`flex-1 h-0.5 mx-2 ${currentStep > idx + 1 ? 'bg-green-600' : 'bg-gray-200'}`} />
                    )}
                  </div>
                ))}
              </div>
            ...
            <div key={exam.id} className="bg-white rounded-lg shadow p-6">
              ...
              <div className="grid grid-cols-4 gap-4 text-sm">
                <div className="bg-gray-50 p-3 rounded">
                  ...
                  <p className="text-xl font-semibold text-gray-900">{exam.question_count}</p>
                </div>
                <div className="bg-gray-50 p-3 rounded">
                  <p className="text-gray-600">Students Assigned</p>
                  <p className="text-xl font-semibold text-gray-900">{exam.assigned_students}</p>
                </div>
                <div className="bg-gray-50 p-3 rounded">
                  <p className="text-gray-600">Completed</p>
                  <p className="text-xl font-semibold text-gray-900">{exam.completed_count}</p>
                </div>
                <div className="bg-gray-50 p-3 rounded">
                  <p className="text-gray-600">Duration</p>
                  <p className="text-xl font-semibold text-gray-900">{exam.duration_minutes}m</p>
                </div>
              </div>
            </div>
          ));
      ...
      {exams.length > examsPerPage && (
        <div className="mt-6 bg-white rounded-lg shadow px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                Showing <span className="font-medium">{((currentPage - 1) * examsPerPage) + 1}</span> to{' '}
                <span className="font-medium">{Math.min(currentPage * examsPerPage, exams.length)}</span> of{' '}
