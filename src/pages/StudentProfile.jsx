              ...
              <div className="flex items-center gap-6">
                <div className="w-24 h-24 rounded-full border-4 border-white bg-white flex items-center justify-center overflow-hidden">
                  {viewingProfile.studentInfo.photo ? (
                    ...
                    <span className="text-4xl font-bold text-primary-600">{viewingProfile.studentInfo.name.charAt(0)}</span>
                  )}
                </div>
                <div className="flex-1">
                  <h1 className="text-3xl font-bold text-white mb-1">{viewingProfile.studentInfo.name}</h1>
                  <p className="text-primary-100 text-lg">Admission No: {viewingProfile.studentInfo.admissionNo}</p>
                </div>
              </div>
              <div className="text-right">
                <div className="bg-white/20 rounded-lg px-4 py-2">
                  <p className="text-primary-100 text-sm">Total Reports</p>
              ...
              <div className="flex flex-wrap gap-2">
                {[...new Set(viewingProfile.reports.map(r => r.class))].map((cls, index, arr) => (
                  <div key={cls} className="flex items-center">
                    <span className="inline-flex px-3 py-1 text-sm font-semibold rounded-full bg-primary-600 text-white">
                      {cls}
                    </span>
                    {index < arr.length - 1 && (
                      <svg className="w-6 h-6 text-primary-400 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div key={sessionClass} className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {reports.sort((a, b) => {
                    ...
                    <div key={report.id} className="border-2 border-gray-200 rounded-lg p-4 hover:border-primary-500 hover:shadow-lg transition-all">
                      ...
                      <div className="flex gap-2 mt-4">
                        <button
                          ...
                          className="flex-1 px-3 py-2 bg-primary-600 text-white text-sm rounded-md hover:bg-primary-700 transition-colors flex items-center justify-center gap-1"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
